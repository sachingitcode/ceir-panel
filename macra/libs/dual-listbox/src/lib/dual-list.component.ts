...
export class DualListComponent implements DoCheck, OnChanges {
	...
	dropitem(event: CdkDragDrop<any[]>) {
		moveItemInArray(this.confirmed.sift, event.previousIndex, event.currentIndex);
		this.confirmed.sift.forEach((value, index) => {
			const ind = _.findIndex(this.destination, {id:value._id});
			this.destination[ind] = Object.assign(this.destination[ind],{displayOrder: index});
		});
	}
}
